...
import withCatalogItems from "containers/catalog/searchWithCatalogItems";
...
class TagGridPage extends Component {
  ...
  render() {
    const { catalogItems, catalogItemsPageInfo, isLoadingCatalogItems, routingStore, shop, tag, uiStore ,searchTitle } = this.props;
    const pageSize =
      routingStore.query && routingStore.query.limit ? parseInt(routingStore.query.limit, 10) : uiStore.pageSize;
    const sortBy = routingStore.query && routingStore.query.sortby ? routingStore.query.sortby : uiStore.sortBy;
    if (!tag && !shop) {
    ...
    const structTitle = `search for "${searchTitle}"`;
      ...
      <Layout shop={shop}>
        ...
        <div className="ContainerContent__MainMaxWidth">
          <div className="ContainerPages__MainMaxWidth">
            ...
            {/* {catalogItems.length > 1 && this.renderFilters()} */}
           { 
            !this.state.nodata&&<SeeAllProducts isFilter={this.state.isFilter} setIsFilter={(val)=>this.setState({isFilter:val})} pageInfo={catalogItemsPageInfo} setPageSize={()=>setPageSize(PAGE_SIZES._20)} catalogItems={catalogItems} displayTitle={structTitle ? structTitle: ""}
