...
import StripePaymentInput from "../../StripePaymentInput/v1";
import AddressChoice from "../../AddressChoice/v1";
...
class PaymentsCheckoutAction extends Component {
  ...
  renderBillingAddressForm() {
    const { addresses,  isSaving, billingAddressTitleText } = this.props;
  ...
  render() {
    ...
    const { selectedPaymentMethodName ,inputIsComplete} = this.state;
    const selectedPaymentMethod = paymentMethods.find((method) => method.name === selectedPaymentMethodName);
    ...
    return (
      <div className={className}>
        ...
        {/*!inputIsComplete ?<p className="mt-5-stripe">
        {" "}
        <small className="inline-error ml-15 ">Enter payment details </small>
    </p>:""*/}
