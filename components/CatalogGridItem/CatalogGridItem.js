...
class CatalogGridItem extends Component {
  ...
  setImageFit = () => {
    ...
    if (typeof Image !== "undefined") {
      ...
      largeImage.onload = () => {
        ...
        const ratio=aspect_ratio>1.6?1:0;
        if (height > width) {
          // Image is portrait
          fit = "contain";
        } else {
  ...
  renderProductMedia() {
    const { classes, components: { ProgressiveImage }, product } = this.props;
    const { fit,ratio } = this.state;
    if(ratio){
      return (
        <div className={classes.media__container}>
  
          <img className={classes.product__thumbnail}  style={{paddingTop:"40px",paddingBottom:"15px"}} src={this.primaryImage.URLs.thumbnail ? this.primaryImage.URLs.thumbnail : "/images/placeholder.gif"} />
          <span className={classes.product__brandLogo}>
            <img className={classes.product__brandImg} src={product.vendor ? `/brand/${product.vendor.toLowerCase().replace(/\s+/g, '').replace("-", "")}.png` : ""} />
          </span>
        </div>
    ...
    return (
      <div className={classes.media__container}>
        ...
        <img className={classes.product__thumbnail} src={this.primaryImage.URLs.thumbnail ? this.primaryImage.URLs.thumbnail : "/images/placeholder.gif"} />
        <span className={classes.product__brandLogo}>
          <img className={classes.product__brandImg} src={product.vendor ? `/brand/${product.vendor.toLowerCase().replace(/\s+/g, '').replace("-", "")}.png` : ""} />
        </span>
      </div>
    ...
    return (
      <div>
        <ProductInfoBottom className="ptb-10 ProductInfoBottom">
          ...
          <PriceContainer >
            <Typography className={classes.product__priceStyle} variant="h6">{pricing && pricing[0]?.displayPrice}</Typography>
