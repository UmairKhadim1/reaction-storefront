...
import Link from "components/Link";
 import CustomNavigationItemMobile from "./CustomNavigationItemMobile";
...
class CustomNavigationMobile extends Component {
  static propTypes = {
  ...
  render() {
    const { classes, navItems, uiStore, shop,authStore:{account} } = this.props;
      ...
      return (
        <Drawer open={uiStore.isMenuDrawerOpen} onClose={this.handleClose}>
              ...
              <div className={classes.toolbarTitle}>
                <Typography className={classes.title} color="inherit" variant="h6">
                  <Link route="/" onClick={this.handleClose}>
                    ...
                    // <img className={classes.logoIcon} src="/icons/logo.png" />
                    }
                  </Link>
                </Typography>
              </div>
          ...
          <nav className={classes.menu}>
            <MenuList disablePadding>{[1].map(this.renderNavItem)}</MenuList>
          </nav>
...
export default withStyles(styles)(inject("navItems", "uiStore")(CustomNavigationMobile));
