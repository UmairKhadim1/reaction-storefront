...
class CatalogGridItem extends Component {
  ...
  renderProductMedia() {
    const {classes, components: { ProgressiveImage }, media } = this.props;
    const { fit } = this.state;
    return (
      <div className={classes.media__container}>
        
          <img className={classes.product__thumbnail} src={media.length>0?media[0].URLs.large:"/images/placeholder.gif"} />
        {/*  <ProductInfo>
    ...
    return (
      <div>
        <ProductInfoBottom className="ptb-10 ProductInfoBottom">
          ...
          <PriceContainer >
             <Typography className={classes.product__priceStyle} variant="h6">{pricing && pricing?.displayPrice}</Typography>
            {/* <Price  displayPrice={productPrice.displayPrice} /> */}
          </PriceContainer>
        </ProductInfoBottom>
        {/* <div>
