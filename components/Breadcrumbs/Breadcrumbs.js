...
class Breadcrumbs extends Component {
  ...
  renderProductNameBreadcrumb = () => {
    const { classes: { breadcrumbIcon, breadcrumbLink }, product, tagId,CurrentVariant,currentVariantId } = this.props;
 ...
 if(CurrentVariant){
  return (
    <Fragment>
      ...
      <Link route={`/product/${product.slug}`}><span className={breadcrumbLink}>{product.title}</span></Link>
      <ChevronRight className={breadcrumbIcon} />
      <Link route={`/variant/${product.slug}?variantId=${currentVariantId}`}><span className={breadcrumbLink}>{CurrentVariant.length>0?CurrentVariant[0].title:""}</span></Link>
    </Fragment>
  );
 }
    return (
  ...
  render() {
    ...
    return (
      <div className={container +" breadcrumb-mob"}>
