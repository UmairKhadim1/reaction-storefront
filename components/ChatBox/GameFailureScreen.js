...
export default function GameSuccessScreen(props) {
    ...
    return (
        <Grid container className="gameFailureScreen">
            
            <Grid className="gameSuccessScreen__innerContainer" item xs={12} sm={10} md={8} lg={5}>
                    <img className="gameSuccessScreen__logo" src=""/>
                    <Typography className="gameSuccessScreen__winnerName" variant="h2">{props.lostByInfo.name} LOST</Typography>
                    <div className="gameSuccessScreen__winnerProfile" style={{backgroundImage:props.lostByInfo.image?"url("+props.lostByInfo.image+")":"url("+"/images/sellerProfile.jpg"+")"}}></div>
