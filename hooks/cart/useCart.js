...
export default function useCart() {
  ...
  const [addOrCreateCartMutation, {
    ...
    onCompleted(addOrCreateCartMutationData) {
      ...
      if (addOrCreateCartMutationData && addOrCreateCartMutationData.createCart && (!viewer || !viewer._id)) {
        ...
        certItem = cartPayload ? cartPayload._id :"";
      }
      const { accountCartId, anonymousCartToken } = cartStore;
      let cartID=!cartData?.cart?._id?addOrCreateCartMutationData?.createCart?.cart?._id:cartData.cart._id;
      if(bidId &&( certItem || accountCartId||cartID))  {
        ...
        var graphqlUpdateCart = JSON.stringify({
          query: `
              ...
              `,
          variables: {
            cartId:cartID ? cartID :"123456789",
            bidId: bidId,
          }
        })
        var requestOptions = {
  ...
  return {
    addItemsToCart: (items,idBid,accessToken) => handleAddItemsToCart({ items,idBid,accessToken }, !cart || !cart._id),
